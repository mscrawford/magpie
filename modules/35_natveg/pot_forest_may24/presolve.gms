...
* beta = fractal edge exponent, n = saturation exponent (edge -> 0 as p -> 1)
...
* Edge ratio: Bodirsky closure predicts E(km) from A(km2) and p
* Convert A from Mha to km2 inline: A_km2 = p35_forest_area * 10000
* Guard: skip when p >= 1 (log10(0) is undefined) or A = 0
  p35_edge_fraction(j) = 0;
  p35_edge_ratio(j) = 0;

  p35_edge_ratio(j)$(p35_forest_area(j) > 1e-10
                      AND f35_edge_intercept(j) > 0
                      AND p35_forest_fraction(j) < 1) =
